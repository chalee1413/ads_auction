...
class DRLearner:
    ...
    def fit(self, X: np.ndarray, treatment: np.ndarray, outcome: np.ndarray):
        ...
        dr_score = ((treatment - e_pred) / (e_pred * (1 - e_pred))) * (outcome - mu_w) + (
            mu_1_pred - mu_0_pred
        )
...
def check_uplift_assumptions(
    ...
    if n_treated < 50:
        ...
        checks["errors"].append(
            f"Treatment group size {n_treated} < 50 (T-learner needs sufficient data)"
        )
    if n_control < 50:
        checks["sufficient_per_group_passed"] = False
    ...
    checks["all_assumptions_passed"] = (
        ...
        and checks["rct_balance_passed"]
        and checks["positivity_passed"]
        and checks["sufficient_per_group_passed"]
    )
...
def evaluate_uplift_models(
    ...
    for name, model in models.items():
        ...
        results[name] = {"mse": mse, "mae": mae, "correlation": correlation}
