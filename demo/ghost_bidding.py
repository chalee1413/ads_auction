...
def ghost_bidding_simulation(
    ...
    control_indices = np.random.choice(eligible_for_control, n_control, replace=False)
    ...
    treatment_cvr = (
        treatment_conversions / treatment_impressions if treatment_impressions > 0 else 0.0
    )
...
def rct_with_ghost_bidding(
    ...
    results = ghost_bidding_simulation(
        impressions, bids, competition_bids, conversion_rates, control_ratio, seed
    )
    ...
    control_total_conversions = np.sum(np.random.binomial(1, base_conversion_rate, control_total))

    # ITT estimate
    itt_effect = results["treatment_cvr"] - (
...
if __name__ == "__main__":
    raise ValueError(
        "This module requires real data. Use with actual bids, competition bids, and conversion rates from datasets."
    )
