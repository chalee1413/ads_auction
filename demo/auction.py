...
def second_price_auction(bids: List[float]) -> Dict[str, any]:
    """
    ...
    return {"winner": int(winner_idx), "winning_bid": float(winning_bid), "payment": float(payment)}


def iroas_based_bidding(
    incremental_value: float, observed_iroas: float = None, target_iroas: float = 100.0
) -> float:
    """
...
def bid_shading(
    valuation: float, competition_level: float, auction_type: str = "first_price"
) -> float:
    """
    Bid shading for first-price auctions.
...
if __name__ == "__main__":
    ...
    competition_bids = [
        np.random.normal(1.5, 0.4, np.random.randint(1, 4)) for _ in range(n_auctions)
    ]
    ...
    print("\nDifference:")
    print(
        f"  Payment Difference: ${roas_results['total_payment'] - iroas_results['total_payment']:.2f}"
    )
    print(
        f"  iROAS Difference: {roas_results['iroas'] - iroas_results['iroas']:.2f} percentage points"
    )
