...
def estimate_propensity_scores(
    X: np.ndarray, treatment: np.ndarray
) -> np.ndarray:
    """
...
def propensity_score_matching(
    ...
    X_matched_control = X_control[matched_control_indices] if len(matched_control_indices) > 0 else X_control[:0]
    
    # Balance diagnostics
    balance = balance_diagnostics(X_treated, X_control, X_matched_control) if len(X_matched_control) > 0 else {}
