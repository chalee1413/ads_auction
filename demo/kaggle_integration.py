...
class KaggleDatasetIntegration:
            ...
            try:
                ...
                if creds and creds.get('username') and creds.get('key'):
                    ...
                    try:
                        ...
                        print(f"   ERROR: Authentication failed after retry: {type(e2).__name__}: {e2}")
                        self.api = None
                else:
    ...
    def download_dataset(
        self,
        dataset_name: str = 'saurav9786/real-time-advertisers-auction',
        unzip: bool = True
    ) -> bool:
        """
    ...
    def load_dataset(
        ...
        if dataset_type == 'auto':
            ...
            auction_files = [f for f in possible_files if 'dataset.csv' in f.name.lower() or 'auction' in f.name.lower()]
            if auction_files:
                return pd.read_csv(auction_files[0])
            
            # Check for video ads files
            video_files = [f for f in possible_files if 'video' in f.name.lower() or 'engagement' in f.name.lower() or 'ad_df.csv' in f.name.lower()]
            if video_files:
        ...
        # Load specific dataset type
            ...
            video_files = [f for f in possible_files if 'video' in f.name.lower() or 'engagement' in f.name.lower()]
            if video_files:
                return pd.read_csv(video_files[0])
    ...
    def process_auction_data(
        ...
        # Extract bid data
        bid_columns = ['auction_id', 'bidder_id', 'bid_price', 'ad_id', 
                      'timestamp', 'advertiser_id', 'placement_id']
        ...
        if 'revenue' in df.columns:
            ...
            processed['revenue'] = processed['revenue'][
                processed['revenue']['revenue'] > 0
            ]
        else:
...
if __name__ == '__main__':
    ...
    print("  result = integrate_kaggle_with_incrementality(dataset_type='auction', dataset_name='saurav9786/real-time-advertisers-auction')")
    ...
    try:
        ...
        print("2. Run: python3 -c \"from demo.kaggle_integration import KaggleDatasetIntegration; k = KaggleDatasetIntegration(); k.download_dataset('saurav9786/real-time-advertisers-auction')\"")
