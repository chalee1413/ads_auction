...
def calculate_incremental_revenue(test_revenue: float, control_revenue: float) -> float:
    """
...
def calculate_iroas(incremental_revenue: float, ad_spend: float) -> float:
    """
...
def calculate_lift(test_metric: float, control_metric: float) -> float:
    """
...
def test_significance(
    test_values: np.ndarray, control_values: np.ndarray, alpha: float = 0.05
) -> Dict[str, float]:
    """
...
def compare_roas_iroas(
    test_revenue: float, control_revenue: float, attributed_revenue: float, ad_spend: float
) -> Dict[str, float]:
    """
    ...
    over_attribution_pct = (
        (over_attribution / attributed_revenue * 100.0) if attributed_revenue > 0 else 0.0
    )
...
if __name__ == "__main__":
    ...
    print(
        f"Over-attribution: ${comparison['over_attribution']:.2f} ({comparison['over_attribution_pct']:.2f}%)"
    )
