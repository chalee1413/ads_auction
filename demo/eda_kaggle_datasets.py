...
def identify_metrics(df, dataset_name):
    ...
    metrics = {
        ...
        "Auction-related": [
            col for col in df.columns if "auction" in col.lower() or "impression" in col.lower()
        ],
        "Click/Conversion": [
            col
            for col in df.columns
            if "click" in col.lower() or "conversion" in col.lower() or "conversion" in col.lower()
        ],
        "Revenue": [
            col
            for col in df.columns
            if "revenue" in col.lower() or "value" in col.lower() or "price" in col.lower()
        ],
        "Time-related": [
            col
            for col in df.columns
        ...
        "User/Session": [
            col
            for col in df.columns
            if "user" in col.lower() or "session" in col.lower() or "id" in col.lower()
        ],
        ...
        "Video/Ad Info": [
            col
            for col in df.columns
            if "ad" in col.lower() or "video" in col.lower() or "campaign" in col.lower()
        ],
    }
...
def create_visualizations(df, dataset_name, iteration=1):
    ...
    if len(numeric_cols) > 0:
        ...
        plt.savefig(
            OUTPUT_DIR
            / f'{dataset_name.lower().replace(" ", "_")}_distributions_iter{iteration}.png',
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
    ...
    if len(numeric_cols) > 1:
        ...
        plt.tight_layout()
        plt.savefig(
            OUTPUT_DIR / f'{dataset_name.lower().replace(" ", "_")}_correlation.png',
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
...
def main():
    ...
    if auction_files:
        ...
        print(
            f"Loaded Real-time Auction dataset: {auction_df.shape[0]} rows, {auction_df.shape[1]} columns"
        )
    ...
    video_files = (
        list(DATA_DIR.glob("video*.csv"))
        + list(DATA_DIR.glob("*video*.csv"))
        + list(DATA_DIR.glob("engagement*.csv"))
        + list(DATA_DIR.glob("*engagement*.csv"))
    )
    ...
    if video_files:
        ...
        print(
            f"Loaded Video Ads Engagement dataset: {video_df.shape[0]} rows, {video_df.shape[1]} columns"
        )
    ...
    if auction_df is not None and video_df is not None:
        ...
        print(
            "   - Real-time Auction dataset: Auction-level data with revenue for iROAS calculations"
        )
        ...
        print(
            "   - Use cases: Combined incrementality measurement across auction revenue and engagement metrics"
        )
