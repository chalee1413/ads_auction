...
def calculate_theta(pre_values: np.ndarray, post_values: np.ndarray) -> float:
    """
...
def check_cuped_assumptions(
    test_pre: np.ndarray, test_post: np.ndarray, control_pre: np.ndarray, control_post: np.ndarray
) -> Dict[str, any]:
    """
    ...
    if len(test_pre) == len(test_post) and len(test_pre) > 1:
        test_corr = (
            np.corrcoef(test_pre, test_post)[0, 1]
            if np.std(test_pre) > 0 and np.std(test_post) > 0
            else 0.0
        )
        control_corr = (
            np.corrcoef(control_pre, control_post)[0, 1]
            if np.std(control_pre) > 0 and np.std(control_post) > 0
            else 0.0
        )
    ...
    size_ratio = (
        min(len(test_pre), len(control_pre)) / max(len(test_pre), len(control_pre))
        if max(len(test_pre), len(control_pre)) > 0
        else 0.0
    )
    ...
    checks["all_assumptions_passed"] = (
        ...
        and checks["pre_post_correlation_passed"]
        and checks["balanced_groups_passed"]
    )
...
def cuped_adjustment(
    ...
    variance_reduction = (
        1.0 - (theta**2 * var_pre / pooled_post_var) if pooled_post_var > 0 else 0.0
    )

    # Effective sample size increase
    effective_sample_size_multiplier = (
        1.0 / (1.0 - variance_reduction) if variance_reduction < 1.0 else 1.0
    )
    ...
    se_adjusted = (
        se_unadjusted * np.sqrt(1.0 - variance_reduction)
