...
def pre_period_balance_test(
    ...
    if len(X.shape) > 1:
        for i in range(X.shape[1]):
            ...
            pooled_std = np.sqrt((np.var(X_treat[:, i], ddof=1) + np.var(X_control[:, i], ddof=1)) / 2)
...
def rct_design(
    ...
    if balance_check and features is not None:
        ...
        all_balanced = all(
            result['is_balanced'] for result in balance_results.values()
        )
